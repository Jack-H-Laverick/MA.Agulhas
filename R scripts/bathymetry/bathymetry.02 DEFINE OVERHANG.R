...
Elevation_In[Elevation < -SDepth] <- -SDepth
Elevation_Off <- Elevation
Elevation_Off[Elevation < -DDepth] <- -DDepth
...
Domains <- mutate(Domains, Elevation = case_when(
    Shore == "Offshore" ~ Offshore_elevation,
